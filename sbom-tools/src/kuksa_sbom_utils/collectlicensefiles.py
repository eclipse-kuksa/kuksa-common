...
import kuksa_sbom_utils.util.maplicensefile as MAP
...
def collect(sbom, targetpath):
    ...
    for license_id in license_ids:
        if license_id not in MAP.supported_licenses:
            ...
            logger.error("This tool currently supports the following license identifiers: %s",
                         MAP.supported_licenses.keys())
            sys.exit(-3)

        logger.info("Copying %s", MAP.supported_licenses[license_id][:-3])
        with gzip.open(os.path.join(LICENSE_STORE, MAP.supported_licenses[license_id]), "rb") as inf:
            content = inf.read()
        with open(os.path.join(targetpath, MAP.supported_licenses[license_id][:-3]), "wb") as outf:
            outf.write(content)
